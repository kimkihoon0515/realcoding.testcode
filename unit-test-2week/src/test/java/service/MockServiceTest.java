...
@RunWith(MockitoJUnitRunner.class) //초기화 setup
public class MockServiceTest {
    ...
    public void DoNothing(){
        Champion champion = mock(Champion.class);
        doNothing().when(champion).setName(anyString());
        champion.setName(anyString());
    }

    @Test
    public void Blitz(){
        Champion champion = mock(Champion.class);
        when(champion.getName()).thenReturn("블츠");
        assertThat(champion.getName(),is("블츠"));
    }

    // 1. when, thenReturn을 사용하여 어떠한 챔피언 이름을 입력해도 베인을 리턴하도록 테스트하세요
    @Test
    public void AlwaysReturnVain(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void TestChampionSize(){
        List<Champion>ChampionsList = mock(List.class);
        Champion champion = mock(Champion.class);
        champion.setName("유미");
        champion.setHasSkinCount(2);
        ChampionsList.add(champion);
        System.out.println("Size :: " + ChampionsList.size());
        verify(ChampionsList,times(1)).size();
    }

    // 4-1. champion 객체에서 이름을 가져오는 로직이 2번 이상 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void TestChampionsSize(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void GetNameHasNotExcuted(){
        Champion champion = mock(Champion.class);
        champion.setName("세트");
    ...
    public void Verify() {
        ...
        champion.setHasSkinCount(1);
        champion.setPosition("탑");
        verify(champion,atLeastOnce()).setName("오른");;
    }


    // 1. 리산드라라는 챔피언 이름으로 검색하면 미드라는 포지션과 함께 가짜 객체를 리턴받고, 포지션이 탑이 맞는지를 테스트하세요
    @Test
    public void Lisandra(){
        when(mockService.findByName("리산드라")).thenReturn(new Champion("리산드라","탑",0));
    ...
    @Test
    public void Bard(){
        when(mockService.findByName("바드")).thenReturn(new Champion("바드","서포터",2));
        int skinCount = mockService.findByName("바드").getHasSkinCount();
        assertThat(skinCount,is(2));
        verify(mockRepository,times(1)).findByName("바드");
    }
    ...
    public void ChampionList(){
        ...
        champions.add(new Champion("바드","서폿",4));
        champions.add(new Champion("소라카","서폿",0));
    ...
    @Test
    public void FakeChampions(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("바이","정글",3));
        champions.add(new Champion("쉔","탑",1));
        when(mockService.findAllChampions()).thenReturn(champions);
        assertThat(mockService.findAllChampions().get(0).getName(),is("바이"));
        assertThat(mockService.findAllChampions().size(),is(2));
        verify(mockRepository,times(2)).findAll();
    }

    @Test
    public void FakerChampions(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("르블랑","미드",10));
        champions.add(new Champion("아지르","미드",1));
        champions.add(new Champion("갈리오","미드",2));
        when(mockService.findAllChampions()).thenReturn(champions);
        assertThat(mockService.findAllChampions().get(0).getName(),is("르블랑"));
